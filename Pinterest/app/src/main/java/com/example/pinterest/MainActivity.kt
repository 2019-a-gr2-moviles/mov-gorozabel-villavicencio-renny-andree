...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val posiciones = arrayListOf<Boolean>(false,false,false,false,false,false,false,false,false,false)
        val recycler_view=rv_example
        val activity = this

        val envio = this.intent.getIntExtra("envio",-1)
        if(envio!=0){
            ...
            val datos = this.intent.getParcelableArrayListExtra<Imagenes>("datos")
            iniciarRV(datos,this,recycler_view)
        }
