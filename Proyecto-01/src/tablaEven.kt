...
class tablaEv(action:String,data:String):JFrame(){
    ...
    private fun createUI(action:String,data:String){
        ...
        val labelH = JLabel("Nombre                     Fecha                   Servant1                  Servant2")
        ...
        botonMostrar.addActionListener {
            ...
            if(table.selectedRow==-1 || table.selectedRow==totalRow)
                ...
                val registro=Evento(seleccionado.get(0),seleccionado.get(1), arrayListOf(seleccionado[2],seleccionado[3]))
...
fun consultarEvent(registro:String,action:String){
    val frame = tablaEv(action,registro)
